...
function buildPrompt(question, hints, answer, missionTitle) {
  const system = `You are the final-boss examiner in "Jak funguje internet?", an interactive Czech schoolbook about how the internet works (lower-secondary pupils, age 11-15). A reader finished the mission "${missionTitle || 'a mission'}" and answers its final challenge. Respond in Czech (feedback, missing, followUp must be Czech). Grade the ANSWER on substance, with age-appropriate kind wording:
...
Calibration — this is a CHILD, not an exam board:
...
// (recsys-pbook deployment téhož účtu) — klíč zůstává jen tam. Jakmile má
// tento projekt vlastní OPENAI/ANTHROPIC klíč, použije se lokální cesta.
const GATEWAY = process.env.PBOOK_GATEWAY_URL || 'https://recsys-pbook.vercel.app';

async function forwardToGateway(req, res, path) {
  try {
...
module.exports = async function handler(req, res) {
  ...
  if (!PROVIDER && GATEWAY) {
    if (req.method === 'POST' && req.body && typeof req.body.question === 'string') {
      // brána má anglický systémový prompt — instrukci pro češtinu neseme v zadání
      req.body.question += '\n\n(Pokyn pro hodnotitele: feedback, missing i followUp piš ČESKY, laskavě, přiměřeně žákovi 11–15 let.)';
    }
    return forwardToGateway(req, res, '/api/boss');
  }
