...
class PBook {
  ...
  async init() {
    ...
    if (location.search.includes('seed=demo') && !localStorage.getItem('pbook-demo-seeded')) {
      ...
      const ids = ['ch1-servery','ch1-klient-server','ch1-kabely','ch1-mapa-kabelu','ch1-routery','ch1-pakety','ch1-obsah-paketu','ch1-nema-reditele','ch1-komiks-nema-reditele','ch1-shrnuti','ch2-wifi-router','ch2-kabely'];
      const now = Date.now();
      const recall = {};
      ...
      localStorage.setItem('pbook-user', JSON.stringify({
        readBlocks: ids, seenBlocks: ids, savedBlocks: ['ch2-satelity-starlink'], ratings: [], notes: [],
        signals: {}, totalInteractions: 40, currentChapter: 1, xp: 175, level: 4,
        achievements: [{ id: 'first_read' }, { id: 'reader_5' }], recall,
        completedMissions: ['m1'], missionTitles: { m1: 'Zvědavý začátek' },
        facetAffinity: { lens: { home: 4 }, depth: { standard: 3 } }, steerPrefs: {}, goal: 'understand', readerMode: 'open',
      }));
  ...
  async toggleTellings(blockId, missMsg, presetTarget) {
    ...
    }).join('')}<span class="tstrip-hint">podání: ${pool.length} — najeď myší pro detail, klikni a čti</span><button class="tstrip-chip" style="--sc:#EC4899" title="Napsat vlastní podání (kouč)" onclick="app.startAuthoring('${conceptId}')">✍️</button></div>`;
  ...
  async renderHome() {
    ...
    // „Protože jsi četl X" — související k poslednímu přečtenému (Items to Item;
    // scénář related-item po založení v Admin UI zpřesní logiku, do té doby default).
    ...
    if (lastReadBlock) {
      ...
      if (relCards.length >= 3) html += this.shelf('Protože sis přečetl/a: ' + this.escHtml(lastReadBlock.meta.title || ''), relCards);
    }
  ...
  async _getNextFeedBlocks(count) {
    ...
    // Infinite feed = JEDNO doporučení stránkované přes Next Items (scénář
    // next-read existuje v DB); nový dotaz jen na začátku nebo po vyčerpání.
  ...
  // Uzel mapy podle slugu NEBO podle concept/block id z jeho poolu
  _cmapNode(id) { return this._cmapNodes?.[id] || this._cmapByPool?.[id] || null; }
  // Bloky patřící uzlu mapy: pool položky se řeší nejdřív jako id bloku (přesné),
  // pak jako koncept (i s variantami); bez poolu spadne na koncept = slug.
  _nodePool(node) {
    if (!node) return [];
  ...
  // Sdílená geometrie herního pole (Cesta) — používá ji velká mapa i plovoucí
  // mini-mapa ve čtení, aby si tvarem odpovídaly.
  _journeyLayout(cm) {
  ...
  async renderJourneyMap() {
    ...
    cm.nodes.forEach(n => (n.prereq || []).forEach((pr, pi) => {
      ...
      arrows += `<path d="M${a.x} ${a.y} Q ${mx} ${my}, ${b.x} ${b.y}" fill="none" stroke="#94A3B8" stroke-width="1.6" stroke-dasharray="5 5" marker-end="url(#jarr)" opacity="0.55">${why ? `<title>Proč nejdřív: ${this.escHtml(why)}</title>` : ''}</path>`;
    }));
    const R = 25;
    ...
    cm.nodes.forEach(n => {
      ...
      if (st.ghost) {
        ...
        if (st.game) svg += `<g style="cursor:pointer" onclick="app.switchView('read');app.openBlock('${st.game}')">
          ...
          <text x="${x + 27}" y="${y + 4}" text-anchor="middle" font-size="10"><title>hra k pojmu</title>🎮</text></g>`;
        return;
      }
      ...
      if (st.game) {
        svg += `<g style="cursor:pointer" onclick="app.switchView('read');app.openBlock('${st.game}')">
          ...
          <text x="${x + R + 12}" y="${y + 4}" text-anchor="middle" font-size="10"><title>hra k pojmu</title>🎮</text>
        </g>`;
      }
    });
    svg += '</svg>';
  ...
  async renderConceptsMap() {
    ...
    let h = `<div class="fade-up" style="max-width:760px;margin:0 auto">
      <p style="font-size:.78rem;color:var(--text-2);margin:.2em 0 .8em">${'Kniha podle <b>konceptů</b>, ne kapitol: každý koncept má svá podání (článek, hra, komiks…) — klikni na ikonku a čti. Tužkou ✍️ rozpracuješ vlastní podání s AI koučem. Čárkované 🌱 koncepty zatím napsané nejsou — hlasuj, kam jít do hloubky. Přečteno <b>{r}/{n}</b> konceptů.'.replace('{r}', readN).replace('{n}', cm.nodes.length)}</p>`;
    cm.temata.forEach(t => {
      const nodes = cm.nodes.filter(n => n.tema === t.id);
      ...
      h += `<details class="covmap-chapter-sec" open>
        <summary style="display:flex;align-items:center;gap:.5em;cursor:pointer"><span style="width:10px;height:10px;border-radius:50%;background:${t.color};flex-shrink:0"></span>
          ...
          <span style="color:var(--text-3);font-size:.72rem;margin-left:auto">${readCount}/${nodes.length} přečteno${ghosts.length ? ' · 🌱 ' + ghosts.length + ' návrhů' : ''}</span></summary>
        ...
        ghosts.forEach(g => {
          ...
          h += `<div style="display:flex;align-items:flex-start;gap:.5em;padding:.22em 0">
            ...
            <span id="ghost-${g.slug}-cmap" style="flex-shrink:0;display:flex;gap:.25em">${voted
              ? `<span style="font-size:.68rem;color:#0EA5E9">${votes[g.slug] > 0 ? '✓ chci' : '✓ nezájem'}</span>`
              : `<button class="steer-chip" style="border-color:#0EA5E9;color:#0EA5E9;font-size:.62rem;padding:.06em .4em" onclick="app.ghostVote('${g.slug}',1,'cmap')">👍 chci</button>
                 <button class="steer-chip" style="font-size:.62rem;padding:.06em .4em" onclick="app.ghostVote('${g.slug}',-1,'cmap')">ne</button>`}
              <button class="steer-chip" style="border-color:#EC4899;color:#EC4899;font-size:.62rem;padding:.06em .4em" onclick="app.startAuthoring('${g.slug}')">✍️ napsat</button>
              <button class="steer-chip" style="border-color:#7C3AED;color:#7C3AED;font-size:.62rem;padding:.06em .4em" title="Tvůrčí dílna s koučem: domluva → tvorba → certifikát s tvým přínosem" onclick="app.startWorkshop('${g.slug}')">🎓 dílna</button></span>
          </div>`;
        });
      }
      h += `</div></details>`;
  ...
  // Plovoucí miniatura herního pole — malá, neinvazivní, pořád vidět při čtení;
  // klik otevře Cestu. Nahrazuje dřívější pruh teček (byl moc na očích).
  // Který pojem právě čteš — mini-mapka ho zvýrazní a mění se při scrollování feedem
  ...
  _refreshMiniBoard() {
    ...
    cm.nodes.forEach(n => {
      ...
      const isFocus = this._feedFocusSlug === n.slug;
      const tip = `<title>${this.escHtml(n.short || n.title)}${isFocus ? ' · právě čteš' : ''}</title>`;
  ...
  _prereqBanner(block) {
    ...
    return `<div class="prereq-banner" style="display:flex;flex-wrap:wrap;gap:.35em;align-items:center;margin:0 0 .6em;padding:.45em .6em;border:1.5px dashed #0EA5E9;border-radius:9px;background:color-mix(in srgb, #0EA5E9 6%, transparent);font-size:.72rem">
      <b>🧩 Aby tohle dávalo smysl, hodí se nejdřív:</b>${chips}
      <a href="#" style="margin-left:auto;color:var(--text-3)" onclick="event.preventDefault();try{const k='pbook-prereq-dismissed';const a=JSON.parse(localStorage.getItem(k)||'[]');a.push('${block.concept}');localStorage.setItem(k,JSON.stringify(a));}catch(e){};this.closest('.prereq-banner').remove()">číst i tak</a>
    </div>`;
  }
  ...
  async renderMap() {
    ...
    if (this._vmapAvailable === undefined) {
      this._vmapAvailable = null;   // probíhá zjištění
    ...
    let html = `<div class="map-header fade-up">
      ...
      <div class="map-mode-toggle">
        ...
        <button class="map-mode-btn ${mapMode === 'cesta' ? 'active' : ''}" onclick="app.setMapMode('cesta')">🎲 Cesta</button>
        ${this._vmapAvailable !== false ? `<button class="map-mode-btn ${mapMode === 'visual' ? 'active' : ''}" onclick="app.setMapMode('visual')">Vizuální</button>` : ''}
  ...
  // Statistika tvůrčího procesu (pro dílnu/certifikát): kolik udělal člověk, kolik AI
  _studioStat(kind) {
  ...
  <div style="margin-top:.45em;border-top:1px dashed var(--border,#eee);padding-top:.4em">
        ...
        <div style="display:flex;gap:.5em;align-items:center;flex-wrap:wrap">
          ...
          <button class="steer-chip" id="stFbBtn" title="Požádej o druhý pár očí: sdílíš snímek draftu (jen ke čtení), ostatní komentují — píšeš pořád jen ty." onclick="app.studioFeedbackPanel()" style="font-size:.72rem">👀 Zpětná vazba</button>
    ...
    blocks.forEach((b, i) => {
      ...
      h = h.replace(this._studioMarkerRx(), (_, k, w) => {
        ...
        return `<span style="display:inline-block;max-width:100%;border:1.5px dashed #7C3AED;border-radius:10px;padding:.35em .6em;margin:.15em .1em;color:#7C3AED;font-size:.78rem;vertical-align:middle">🎨 ${/anim/i.test(k) ? 'ANIMACE' : 'DIAGRAM'}: ${this.escHtml(w.trim())}
          <button class="steer-chip" style="font-size:.64rem;margin-left:.5em;border-color:#7C3AED;color:#7C3AED" onclick="app.generateGraphic(${mi})">🎨 Nakreslit tenhle · ${CONFIG.aiEconomy?.prices.advanced || 0} ⚡</button></span>`;
      });
  ...
  // Klik na nadpis = přejmenování podání (ukládá se k draftu)
  _studioEditTitle() {
    const stdo = this._studio; if (!stdo) return;
  ...
  // ===== TVŮRČÍ DÍLNA — oddělený průvodce nad studiem =====
  // role → domluva s koučem (alignment, mode coach/phase align) → tvorba ve
  // studiu → reflexe (co přinesl člověk) → certifikát s lidským přínosem.
  // Studio zůstává rychlý editor; dílna je pomalá, vědomá cesta.
  ...
  _wsRender() {
    ...
    if (ws.step === 'role') {
      ...
      inner = `<h3 style="margin:.4em 0">Jakou roli chceš mít?</h3>
      ...
      inner = `<h3 style="margin:.4em 0">Domluva s koučem <span style="font-size:.7rem;color:var(--text-3);font-weight:400">(${ROLE_NAMES[ws.role] || ''})</span></h3>
        <div id="wsChat" style="border:1.5px solid var(--border,#ddd);border-radius:12px;padding:.6em .7em;background:var(--card,#fff);max-height:46vh;overflow-y:auto">${msgs || ''}
          ${!ws.msgs.length ? `<button class="steer-chip" onclick="app._wsAsk('')">🎧 Nech kouče začít · ${CONFIG.aiEconomy?.prices.basic || 0} ⚡</button>` : ''}
          <div id="wsSpin"></div></div>
        <div style="display:flex;gap:.4em;margin-top:.5em">
          <input id="wsInput" placeholder="Napiš kouči… (Enter = poslat)" style="flex:1 1 auto;min-width:0;font:inherit;font-size:.88rem;padding:.5em .6em;border:1.5px solid var(--border,#ddd);border-radius:10px;background:var(--card,#fff)">
          <button class="note-save" style="background:var(--accent)" onclick="app._wsAsk(document.getElementById('wsInput').value)">Poslat · ${CONFIG.aiEconomy?.prices.basic || 0} ⚡</button>
        </div>
      ...
      inner = `<h3 style="margin:.4em 0">Tvoje stopa v díle</h3>
        <p style="font-size:.78rem;color:var(--text-2)">Certifikát doloží, co jsi do tvorby vnesl/a TY. Napiš to vlastními slovy.</p>
        <div style="font-size:.75rem;color:var(--text-2);margin:.3em 0">✋ ${stats.manual} ručních úprav · ✨ ${stats.ai} kroků s AI · 🧭 ${stats.coach} kol s koučem</div>
        <label style="font-size:.75rem;font-weight:700">Co jsem do tvorby přinesl/a já (nápad, námitky, příklady, úpravy…)</label>
        ...
        <label style="font-size:.75rem;font-weight:700;margin-top:.5em;display:block">Co jsem se při tom naučil/a</label>
        ...
        <div style="margin-top:.6em"><button class="note-save" style="background:#10B981" onclick="app._wsFinish()">Dokončit → certifikát</button></div>`;
      ...
      inner = `<div style="border-top:6px solid #7C3AED;border-radius:14px;background:var(--card,#fff);padding:1em 1.2em" id="wsCert">
        <div style="font-size:1.1rem;font-weight:800">🎓 Certifikát z tvůrčí dílny</div>
        <input value="${this.escHtml(name)}" placeholder="Tvoje jméno (pro tisk)" onchange="localStorage.setItem('pbook-cert-name',this.value)"
        ...
        <div style="font-size:.78rem;margin:.4em 0">✋ ${stats.manual} ručních úprav · ✨ ${stats.ai} kroků s AI · 🧭 ${stats.coach} kol s koučem</div>
        ${ws.reflect.brought ? `<div style="font-size:.82rem;margin:.4em 0"><b>Co jsem přinesl/a:</b> ${this.escHtml(ws.reflect.brought)}</div>` : ''}
        ${ws.reflect.learned ? `<div style="font-size:.82rem;margin:.4em 0"><b>Co jsem se naučil/a:</b> ${this.escHtml(ws.reflect.learned)}</div>` : ''}
        ...
        <div style="display:flex;gap:.5em;margin-top:.7em" class="no-print">
          <button class="note-save" onclick="print()">🖨 Vytisknout</button>
          ${ws.blockId ? `<button class="note-save" style="background:var(--accent)" onclick="document.getElementById('workshop').remove();app._showShareConsent('${ws.blockId}')">📣 Sdílet se třídou</button>` : ''}
  ...
  // ===== DRUHÝ PÁR OČÍ: feedback na snapshot draftu (NE spolueditace) =====
  // Autor sdílí snímek (#zpetna-vazba-<id>), pomocník jen čte a komentuje
  // (👍/❓/💡 — stejný jazyk jako kouč). Autorství i statistiky zůstávají čisté.
  studioFeedbackPanel() {
    const stdo = this._studio; if (!stdo) return;
  ...
  async _studioFbStatus(shareId) {
    ...
    out.innerHTML = `<div style="border:1.5px solid var(--accent);border-radius:10px;padding:.6em .8em;font-size:.8rem;background:var(--card,#fff)">
      <b>💬 Zpětná vazba (${comments.length})</b>
      <button class="steer-chip" style="font-size:.66rem" onclick="app._studioFbStatus('${shareId}')">↻ obnovit</button>
      <button class="steer-chip" style="font-size:.66rem" onclick="app._studioFbUnshare('${shareId}')">🗑 stáhnout sdílení</button>
      <button class="steer-chip" style="font-size:.66rem" onclick="document.getElementById('stOut').innerHTML=''">✕</button>
      <div style="font-size:.7rem;margin:.35em 0;display:flex;gap:.4em;align-items:center;flex-wrap:wrap"><span style="color:var(--text-2)">Odkaz vytvořen — pošli ho, komu chceš:</span>
        <code style="font-size:.66rem;background:var(--bg,#f6f6f2);padding:.1em .4em;border-radius:6px;word-break:break-all">${link}</code>
        <button class="steer-chip" style="font-size:.64rem" onclick="navigator.clipboard&&navigator.clipboard.writeText('${link}')">Kopírovat</button></div>
      ${comments.length ? comments.map(c => `<div style="border-top:1px dashed var(--border,#eee);padding:.3em 0;font-size:.78rem">${KIND[c.kind] || '💬'} ${this.escHtml(c.text)} <span style="color:var(--text-3);font-size:.66rem">— ${this.escHtml(c.nick || 'anonym')}</span></div>`).join('')
        : `<div style="color:var(--text-3);font-size:.72rem">Zatím žádné komentáře — odkaz je aktivní.</div>`}
    </div>`;
  }
  ...
  async openDraftFeedback(shareId, optimistic) {
    ...
    const KIND = { works: '👍', question: '❓', idea: '💡' };
    const inner = !data?.ok
      ? `<div style="font-size:.85rem;color:var(--text-2)">Tenhle odkaz už není aktivní (autor sdílení stáhl, nebo neexistuje).</div>`
      : `<div style="font-size:.75rem;color:var(--text-2);margin:.2em 0 .6em">Tohle je rozdělaný text — jen ke čtení. Pomoz komentářem: co funguje, čemu nerozumíš, co bys zkusil/a jinak.</div>
        ...
        <textarea id="fbText" placeholder="Napiš komentář…" style="width:100%;min-height:8vh;font:inherit;font-size:.85rem;padding:.5em;border:1.5px solid var(--border,#ddd);border-radius:10px;background:var(--card,#fff)"></textarea>
        <div style="display:flex;gap:.4em;margin-top:.4em;flex-wrap:wrap">
          <input id="fbNick" placeholder="Tvoje přezdívka (nepovinné)" style="flex:1 1 130px;min-width:0;font:inherit;font-size:.8rem;padding:.3em .5em;border:1px solid var(--border,#ddd);border-radius:8px">
          <button class="note-save" style="background:var(--accent)" onclick="app._sendDraftComment('${this.escHtml(shareId)}')">Odeslat komentář</button>
        </div>
        ${data.comments.length ? `<div style="margin-top:.6em;font-size:.76rem">${data.comments.map(c => `<div style="border-top:1px dashed var(--border,#eee);padding:.3em 0">${KIND[c.kind] || '💬'} ${this.escHtml(c.text)} <span style="color:var(--text-3);font-size:.66rem">— ${this.escHtml(c.nick || 'anonym')}</span></div>`).join('')}</div>` : ''}`;
    el.innerHTML = `<div style="position:fixed;inset:0;background:rgba(20,20,30,.5);z-index:270;overflow-y:auto" onclick="if(event.target===this)document.getElementById('draftFb').remove()">
      <div style="max-width:680px;margin:3vh auto 6vh;background:var(--card,#fff);border:1px solid var(--border,#ddd);border-radius:16px;box-shadow:0 14px 44px rgba(0,0,0,.28);padding:1em 1.2em 1.4em">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:.6em">
          <div style="font-weight:800;font-size:1rem">👀 ${data?.ok ? this.escHtml(data.draft.nick || 'anonym') + ' prosí o zpětnou vazbu' : 'prosí o zpětnou vazbu'}</div>
          <button onclick="document.getElementById('draftFb').remove()" title="Zavřít" style="width:34px;height:34px;flex-shrink:0;border-radius:50%;border:1.5px solid var(--border,#ccc);background:var(--bg,#fafaf7);font-size:1rem;font-weight:700;cursor:pointer;line-height:1">✕</button>
        </div>
        ${inner}
      </div></div>`;
    document.body.appendChild(el);
  ...
  // Dvojklik na <text> = přepis popisku v plovoucím inputu přímo u textu (žádný prompt)
  _studioLabelEdit(e) {
  ...
  // ✨ AI úprava obrázku: instrukce se píše v inline řádku pod tlačítky (žádný prompt)
  ...
  // Odebrání obrázku bez potvrzovacího okna — místo něj okamžité [↩ Vrátit]
  ...
  // 🎨 Generování grafiky: KAŽDÁ značka v textu = jeden obrázek; kreslí se
  // postupně (kredit po kreditu), každý hotový hned nahradí svou značku.
  // Bez značek se nakreslí jedno shrnutí textu. onlyIdx = jen jedna značka.
  async generateGraphic(onlyIdx) {
    const stdo = this._studio; if (!stdo) return;
    const out = document.getElementById('stOut');
    const ta = document.getElementById('stDraft');
    const draft = (ta?.value || '').trim();
    if (draft.length < 40) { out.innerHTML = `<div style="background:#FEF3C7;border-radius:8px;padding:.5em .7em;font-size:.8rem">Napiš nejdřív text (aspoň pár vět) — grafika se kreslí z něj.</div>`; return; }
    ...
    if (marks.length > 1 && !window.__stForce) {
      ...
      out.innerHTML = `<div style="display:flex;gap:.5em;align-items:center;flex-wrap:wrap;border:1.5px solid #D97706;border-radius:10px;padding:.45em .6em;font-size:.8rem;background:var(--card,#fff)">
        🎨 Našel jsem {n} značek pro obrázky (~{sum} ⚡ celkem).
        <button class="steer-chip" style="border-color:#D97706;color:#B45309;font-weight:700" onclick="window.__stForce=1;app.generateGraphic().finally(()=>{window.__stForce=0})">🎨 Nakreslit vše</button>
        <span style="color:var(--text-3);font-size:.72rem">nebo klikej na značky v náhledu po jedné</span>
        <button class="steer-chip" onclick="document.getElementById('stOut').innerHTML=''">✕</button>
    ...
    for (let i = 0; i < jobs.length; i++) {
      ...
      const pay = this.aiCanPay('advanced');
      if (!pay.ok) {
        out.innerHTML = (ok ? `<div style="font-size:.78rem;margin-bottom:.4em">Došly ⚡ — vygenerováno {ok} z {n}. Zbylé značky můžeš nakreslit později kliknutím v náhledu.</div>`.replace('{ok}', String(ok)).replace('{n}', String(jobs.length)) : '') + this.aiPaywallHtml('advanced');
        if (btn) { btn.disabled = false; btn.textContent = orig; }
        return;
      }
      if (btn) btn.textContent = 'Kreslím {i}/{n}…'.replace('{i}', String(i + 1)).replace('{n}', String(jobs.length));
      out.innerHTML = `<span class="gen-spinner">🎨 Kreslím {i}/{n}: „{w}"</span>`
        .replace('{i}', String(i + 1)).replace('{n}', String(jobs.length))
        .replace('{w}', this.escHtml((job.wish || 'Nakresli jednoduchý diagram, který shrne hlavní myšlenku tohoto textu').slice(0, 80)));
      const instruction = job.wish ? `${job.wantAnim ? 'animace' : 'diagram'}: ${job.wish}` : 'Nakresli jednoduchý diagram, který shrne hlavní myšlenku tohoto textu';
      try {
        const res = await fetch(CONFIG.steering.generateEndpoint, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
    ...
    out.innerHTML = `<div style="border:1.5px solid ${failed.length ? '#D97706' : '#10B981'};border-radius:8px;padding:.5em .7em;font-size:.8rem;background:var(--card,#fff)">`
      + '✅ Vloženo obrázků: {ok}{fail}'.replace('{ok}', String(ok)).replace('{fail}', failed.length ? ' · bez výsledku: {f} (zkus značku upřesnit a klikni na ni v náhledu)'.replace('{f}', failed.map(f => `„${this.escHtml(f)}"`).join(', ')) : '')
      + '</div>';
    if (btn) { btn.disabled = false; btn.textContent = orig; }
  }
  ...
  async seedDraft() {
    ...
    if (ta.value.trim().length > 60 && !window.__stSeedForce) {
      out.innerHTML = `<div style="display:flex;gap:.5em;align-items:center;flex-wrap:wrap;border:1.5px solid var(--accent);border-radius:10px;padding:.45em .6em;font-size:.8rem;background:var(--card,#fff)">
        ✨ Kostra přepíše tvůj rozepsaný text.
        <button class="steer-chip" style="border-color:var(--accent);color:var(--accent);font-weight:700" onclick="window.__stSeedForce=1;app.seedDraft().finally(()=>{window.__stSeedForce=0})">Přepsat kostrou</button>
        <button class="steer-chip" onclick="document.getElementById('stOut').innerHTML=''">✕</button>
      </div>`;
      return;
    }
    ...
    btn.disabled = true; const orig = btn.textContent; btn.textContent = 'Kouč píše kostru…';
    try {
      const res = await fetch(CONFIG.steering.generateEndpoint, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
      ...
      out.innerHTML = `<div style="border:1.5px dashed var(--accent);border-radius:10px;padding:.6em .8em;font-size:.8rem;background:var(--card,#fff)">
        <b>🌱 Tohle je záměrně děravá kostra — kouč ji nechal nedodělanou. Doplň [DOPLŇ] mezery, přepiš po svém, škrtej. Autorem jsi ty.</b>
        ${data.questions?.length ? `<div style="margin-top:.35em"><b>Otázky ke kostře:</b><ul style="margin:.2em 0 0 1.1em">${data.questions.map(q => `<li>${this.escHtml(q)}</li>`).join('')}</ul></div>` : ''}
      </div>`;
    } catch (e) {
      const pw = this._aiErrorPaywall(e, 'basic');
      out.innerHTML = pw || `<div style="background:#FEE2E2;border-radius:8px;padding:.5em .7em;font-size:.8rem">${this.escHtml(e.message)}</div>`;
  ...
  finishAuthoring() {
    ...
    this.showXPToast('Uloženo do knihy ✍️', 'achievement');
  ...
  aiPaywallHtml(tier, kind) {
    ...
    if (kind === 'login' || (!this._getAuth() && this._trialUsedLocal())) {
      return `<div style="border:1.5px solid #0EA5E9;background:var(--card,#fff);border-radius:10px;padding:.55em .7em;font-size:.74rem;line-height:1.5">
        <b>🔑 AI je pro přihlášené</b><br>Vyzkoušení zdarma už proběhlo. Přihlas se v <a href="#" onclick="event.preventDefault();app.switchView('profile')">Profilu</a> — ⚡ se ti začnou počítat na serveru a dosavadní XP se přenesou (do 100).<br>
        <span style="color:#15803D;font-size:.68rem">Vedeme k hospodárnému využívání AI — ruční práce a přemýšlení se cení víc. 🌱</span></div>`;
    }
  ...
  updateXPBadge() {
    ...
    el.textContent = editor + (reward?.icon || '') + ' Úr. ' + this.user.level + ' · ' + this.user.xp + ' XP' + (CONFIG.aiEconomy?.enabled && this._srvBalance != null ? ' · ⚡' + this._srvBalance : '');
